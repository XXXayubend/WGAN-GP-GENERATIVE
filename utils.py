...
ROOM_CLASS = {"living_room": 1, "kitchen": 2, "bedroom": 3, "bathroom": 4, "couloire": 5, "entrance": 6, "dining room": 7, "study room": 8,
              "storage": 10 , "front door": 15, "murs": 16, "interior_door": 17}
CLASS_ROM = {}
...
def _init_input(graph, prev_state=None, mask_size=64):
    ...
    # unpack
    fixed_nodes = prev_state.get('fixed_nodes', None)
    prev_mks = prev_state.get('masks', None)
    if prev_mks is None:
        prev_mks = torch.zeros((given_nds.shape[0], mask_size, mask_size)) - 1.0
    # initialize masks
    if fixed_nodes is None:
        fixed_nodes_tensor = torch.empty(0, dtype=torch.long)
    else:
        fixed_nodes_tensor = torch.tensor(fixed_nodes, dtype=torch.long)
    given_masks_in = fix_nodes(prev_mks, fixed_nodes_tensor)
